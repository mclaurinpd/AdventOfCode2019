...
def getValue(parameter, mode, array):
    if mode == 0:
...
def miniComp(array):
    ...
    while(array[cursor] != 99):
        opcode, mode1, mode2, mode3 = getModesAndOpcodes(array[cursor])
        param1 = getValue(cursor+1, mode1, array)

        if opcode == 1:
            param2 = getValue(cursor+2, mode2, array)  
            val = param1 + param2
            if mode3 == 0:
                array[array[cursor + 3]] = val
            else:
                array[cursor + 3] = val
            cursor += 4
        elif opcode == 2:
            ...
            if param1 != 0:
                cursor = getValue(cursor+2, mode2, array)
            else:
                cursor+=3
        elif opcode == 6:
            if param1 == 0:
